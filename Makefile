...
lint: ## check style with flake8 and isort
	flake8 tghs tests
	isort -c --recursive tghs tests

.PHONY: fix-lint
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find tghs -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive tghs
	isort --apply --atomic --recursive tghs
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source tghs -m pytest
	coverage report -m
	coverage html
