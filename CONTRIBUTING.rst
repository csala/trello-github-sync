...
Report bugs at https://github.com/csala/trello-github-sync/issues.
...
Trello Github Sync could always use more documentation, whether as part of the
official Trello Github Sync docs, in docstrings, or even on the web in blog posts,
articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/csala/trello-github-sync/issues.
...
Ready to contribute? Here's how to set up `Trello Github Sync` for local development.

1. Fork the `Trello Github Sync` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/trello-github-sync.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv trello-github-sync
...
5. The pull request should work for Python2.7, 3.4, 3.5 and 3.6. Check
   https://travis-ci.org/csala/trello-github-sync/pull_requests
...
2. The tests that cover a module called ``tghs/path/to/a_module.py`` should be
   implemented in a separated module called ``tests/tghs/path/to/test_a_module.py``.
...
2. Update the version in ``setup.cfg``, ``tghs/__init__.py`` and ``HISTORY.md`` files.
3. Create a new TAG pointing at the correspoding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``tghs/__init__.py`` to open the next
